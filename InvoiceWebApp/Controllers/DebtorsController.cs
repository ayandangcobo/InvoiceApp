...
namespace InvoiceWebApp.Controllers
{
    ...
    public class DebtorsController : Controller
    {
        ...
        public async Task<IActionResult> Index(int? page, int? pageSize)
        {
            ...
            {
                //Debtor --> Address
                var hasAddress = debtor.Addresses.FirstOrDefault();
                Address address = null;

                if (hasAddress != null)
                    address = await _addressRepository.GetAddressByPostalAndNumber(hasAddress.Number, hasAddress.PostalCode);
        ...
        public async Task<IActionResult> GetById(string id)
        {
            ...
            //Get user
            var data = new Debtor();
            data =  await _repo.GetDebtorByID(id);
            if (data == null)
            {
                return StatusCode(500, "Debtor could not be found.");
            }
            ...
            //Debtor --> Address
            var hasAddress = data.Addresses.FirstOrDefault();
            ...
            //Debtor model
            var result = new DebtorViewModel();
            if (address != null && addressViewModel != null) // && data. != null)
                result.Address = addressViewModel;
